...
{
	const std::vector<Vector>& s0 = src0.getVertices();
	const std::vector<Vector>& s1 = src1.getVertices();

	dst.clear();
	dst.reserve(s0.size() + s1.size());

	float d[4];
	d[0] = Math::getDistanceSquare(s0.back(), s1.front());
	d[1] = Math::getDistanceSquare(s0.back(), s1.back());
	d[2] = Math::getDistanceSquare(s0.front(), s1.front());
	d[3] = Math::getDistanceSquare(s0.front(), s1.back());
	float nearest = FLT_MAX;
...
void Math::mergeMultiChains(const std::vector<ChainShape*>& src, std::vector<Vector>& dst)
{
	if (src.size() == 0)
	{
		return;
...
{
	const std::vector<Vector>& s0 = chain0.getVertices();
	const std::vector<Vector>& s1 = chain1.getVertices();
